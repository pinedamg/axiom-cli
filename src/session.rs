...
impl AxiomSession {
    pub fn new(config: AxiomConfig) -> anyhow::Result<Self> {
        ...
        let ppid = sysinfo::Pid::from_u32(std::process::id()).as_u32(); // Fallback if we can't get PPID
        ...
        let redactor = PrivacyRedactor::new(config.entropy_threshold, config.pii_patterns.clone());

        // Strategy Selection: Based on consolidated config
        let intelligence: Box<dyn crate::engine::intelligence::IntelligenceProvider> = match config
            .intelligence_mode
        {
            IntelligenceMode::Neural => match NeuralIntelligence::new() {
                Ok(n) => Box::new(n),
                Err(e) => {
        ...
        let mut engine =
            AxiomEngine::new(redactor, schemas, intelligence, config.discovery_threshold);
    ...
    pub fn finalize(
        ...
        crate::engine::telemetry::Telemetry::report_usage(
            &self.config,
            command,
            original,
            compressed,
