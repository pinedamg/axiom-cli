...
impl CommandHandler for NpmHandler {
    ...
    fn generate_insight(&self, _command: &str, buffer: &DiscoveryBuffer) -> Option<String> {
        ...
        if warnings > 10 || audit_issues > 0 {
            ...
            Some(format!(
                "Node.js Installation: Successfully added {} new packages to the project.",
                added
            ))
        } else {
            None
    ...
    fn format_summary(&self, key: &str, items: &[LineMetadata]) -> Option<String> {
        ...
        match *type_label {
            "WARN" => {
                ...
                let suffix = if count > 2 {
                    format!(" and {} more...", count - 2)
                } else {
                    "".to_string()
                };
                Some(format!(
                    "• Hidden {} deprecation warnings (e.g. {}{})",
                    count,
                    examples.join(", "),
                    suffix
                ))
            ...
            "AUDIT" => Some(format!(
                "• Security Audit: {} vulnerabilities found and collapsed.",
                count
            )),
            _ => None,
        }
