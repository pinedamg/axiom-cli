...
impl CommandHandler for PsHandler {
    ...
    fn generate_insight(&self, _command: &str, buffer: &DiscoveryBuffer) -> Option<String> {
        let mut max_cpu = 0.0;
        let mut top_proc: Option<&str> = None; // ⚡ Bolt: Use Option<&str> to avoid redundant String clones inside the iteration loop
        ...
        if total_procs > 0 {
            if max_cpu > 10.0 {
                Some(format!("High CPU load detected: {} is using {}% CPU. Total active processes: {}.", top_proc.unwrap_or(""), max_cpu, total_procs))
            } else {
