...
impl CommandHandler for PsHandler {
    ...
    fn generate_insight(&self, _command: &str, buffer: &DiscoveryBuffer) -> Option<String> {
        let mut max_cpu = 0.0;
        // ⚡ Bolt: Using Option<&str> instead of String to avoid cloning strings in the tight loop
        // when finding the top CPU process, referencing the DiscoveryBuffer directly.
