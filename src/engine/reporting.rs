...
impl EfficiencyReport {
    ...
    fn canonicalize_tool(cmd: &str) -> String {
        ...
        if cmd.starts_with("git") {
            ...
            "cargo".to_string()
        } else if cmd.starts_with("kubectl") {
            "k8s".to_string()
        } else if cmd.starts_with("terraform") {
            "tf".to_string()
        } else {
            "other".to_string()
        }
    ...
    pub fn ratio(&self) -> f64 {
        if self.total_original == 0 {
            ...
            (self.saved_chars() as f64 / self.total_original as f64) * 100.0
        }
    }
...
impl ReportRenderer {
    pub fn render_dashboard(report: &EfficiencyReport) {
        ...
        println!(
            "{:<12} | {:<10} | {:<10} | {:<8}",
            "Tool", "Original", "Saved", "Efficiency"
        );
        println!("{}", "-".repeat(50));
        ...
        println!(
            "  Tokens Avoided:    \x1b[33m~{}\x1b[0m",
            report.saved_chars() / 4
        );
        println!(
            "  Credits Saved:     \x1b[32m~${:.2} USD\x1b[0m (Premium Estimate)",
            report.estimated_usd_saved()
        );
...
mod tests {
    ...
    fn test_canonicalization() {
        ...
        assert_eq!(
            EfficiencyReport::canonicalize_tool("sudo docker ps"),
            "other"
        ); // Note: future improvement for 'sudo'
    }
}
