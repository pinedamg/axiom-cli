...
impl AxiomEngine {
    ...
    fn stage_deduplicate<'a>(&mut self, line: &'a str) -> (Option<String>, PipelineAction<'a>, String) {
        if self.discovery.last_line.as_deref() == Some(line) {
            ...
            // ⚡ Bolt: Reuse the existing Option<String> buffer to avoid allocating a new String per line in the hot path.
            if let Some(ref mut buf) = self.discovery.last_line {
                buf.clear();
                buf.push_str(line);
            } else {
