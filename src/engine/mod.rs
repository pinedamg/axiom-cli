...
use crate::engine::commands::{get_all_handlers, CommandHandler};
...
use crate::schema::{Action, ToolSchema};
...
impl AxiomEngine {
    ...
    pub fn trace_summary(&mut self, summary: &str) {
        if self.dev_mode {
            self.traces.push(LineTrace {
                ...
                events: vec![TraceEvent::Analyzed(
                    "Summary".to_string(),
                    "Synthesized from buffer".to_string(),
                )],
            });
        }
    }
    ...
    pub fn process_line(
        ...
        let mut events = if self.dev_mode {
            Vec::new()
        } else {
            Vec::with_capacity(0)
        };
        ...
        if self.dev_mode {
            events.push(TraceEvent::Deduplicated(reason));
        }
        ...
        let final_output = match action {
            ...
            PipelineAction::Continue(l) => {
                ...
                let (analyze_result, reason, is_buffered) =
                    self.stage_analyze(&redacted, command, context);
                if self.dev_mode {
                    events.push(TraceEvent::Analyzed(
                        format!("{:?}", analyze_result),
                        reason,
                    ));
                    if is_buffered {
                        events.push(TraceEvent::Buffered(
                            "Added to synthesis buffer for future summary".to_string(),
                        ));
                    }
                }
    ...
    fn stage_deduplicate<'a>(
        &mut self,
        line: &'a str,
    ) -> (Option<String>, PipelineAction<'a>, String) {
        if self.discovery.last_line.as_deref() == Some(line) {
            self.discovery.repeat_count += 1;
            (
                None,
                PipelineAction::Swallow,
                "Identical to previous line".to_string(),
            )
        } else {
            let prefix = if self.discovery.repeat_count > 0 {
                Some(format!(
                    "... (previous line repeated {} more times)",
                    self.discovery.repeat_count
                ))
            } else {
                None
            };

            // ⚡ Bolt: Reuse the existing `Option<String>` buffer by extracting it with `.take()`,
            // clearing it, and appending data with `.push_str()` instead of constantly allocating new strings on the heap.
    ...
    fn stage_transform<'a>(&self, line: &'a str) -> (Cow<'a, str>, String) {
        if self.markdown_mode && ContentTransformer::looks_like_table(line) {
            (
                ...
                "Table converted to Markdown".to_string(),
            )
        } else {
    ...
    fn stage_guard<'a>(
        ...
        if ContentTransformer::should_guard(command, self.line_counter, context) {
            if self.line_counter == 101 {
                return (
                    PipelineAction::ShortCircuit(Cow::Owned(
                        "[AXIOM] High noise detected. Activating Guardian Mode...".to_string(),
                    )),
                    "High noise threshold reached (line 101)".to_string(),
                );
            }
            if self.line_counter > 100 && !is_outlier {
                return (
                    PipelineAction::Swallow,
                    "Dropped by Guardian Mode (noise line > 100)".to_string(),
                );
            }
        }
        (
            PipelineAction::Continue(Cow::Borrowed(line)),
            "Allowed by Guardian".to_string(),
        )
    }
    ...
    fn stage_analyze<'a>(
            ...
            if let Some(action) = schema.apply_rules(line) {
                match action {
                    Action::Keep => {
                        return (
                            PipelineAction::Continue(Cow::Borrowed(line)),
                            format!("Schema match ({}): KEEP", schema.name),
                            false,
                        )
                    }
                    Action::Redact => {
                        return (
                            PipelineAction::ShortCircuit(Cow::Owned(
                                "[REDACTED_BY_SCHEMA]".to_string(),
                            )),
                            format!("Schema match ({}): REDACT", schema.name),
                            false,
                        )
                    }
                    Action::Hidden => {
                        return (
                            PipelineAction::Swallow,
                            format!("Schema match ({}): HIDE", schema.name),
                            false,
                        )
                    }
                    Action::Collapse | Action::Synthesize => {
                        ...
                        if self
                            .discovery
                            .process_and_check_noise(line, handler, command)
                        {
                            return (
                                PipelineAction::Swallow,
                                format!(
                                    "Schema match ({}): COLLAPSE (discovered noise)",
                                    schema.name
                                ),
                                true,
                            );
                            ...
                            return (
                                PipelineAction::Continue(Cow::Borrowed(line)),
                                format!("Schema match ({}): KEEP (not noise yet)", schema.name),
                                false,
                            );
                        }
                    }
        ...
        if context.is_relevant(line) {
            return (
                PipelineAction::Continue(Cow::Borrowed(line)),
                "Semantic match (Context keywords)".to_string(),
                false,
            );
        }
        if self
            .intelligence
            .is_relevant(&context.last_message, line, 0.7)
        {
            return (
                PipelineAction::Continue(Cow::Borrowed(line)),
                "Semantic match (LLM Embedding)".to_string(),
                false,
            );
        }
            ...
            .process_and_check_noise(line, handler, command)
        {
            (
                PipelineAction::Swallow,
                "Dropped by General Discovery Engine".to_string(),
                true,
            )
        } else {
            (
                PipelineAction::Continue(Cow::Borrowed(line)),
                "General keep (Low noise)".to_string(),
                false,
            )
        }
    }
    ...
    ) -> (Option<Cow<'a, str>>, Vec<(String, String)>) {
        ...
        match (line, &mut self.plugins) {
            (Some(current), Some(manager)) => {
                ...
                if transformed != *current {
                    reasons.push((
                        "WASM Plugins".to_string(),
                        "Line modified by external plugin".to_string(),
                    ));
                }
    ...
    fn generate_semantic_insight(&self) -> Option<String> {
        if let Some(handler) = self.handlers.iter().find(|h| h.matches(&self.last_command)) {
            if let Some(insight) =
                handler.generate_insight(&self.last_command, &self.discovery.synthesis_buffer)
            {
                return Some(insight);
            }
        }
...
mod tests {
    ...
    fn test_line_tracing() {
        ...
        let has_dedup = traces[0]
            .events
            .iter()
            .any(|e| matches!(e, TraceEvent::Deduplicated(_)));
        assert!(has_dedup);
    }
}
