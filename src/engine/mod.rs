...
impl AxiomEngine {
    ...
    fn stage_deduplicate<'a>(&mut self, line: &'a str) -> (Option<String>, PipelineAction<'a>, String) {
        if self.discovery.last_line.as_deref() == Some(line) {
            ...
            // ⚡ Bolt: Usamos .take(), .clear() y .push_str() para reutilizar el buffer existente de Option<String>.
            // Esto evita la asignación de un nuevo String en el heap por cada nueva línea en el "hot path".
