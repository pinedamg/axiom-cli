...
fn test_learning_lifecycle_and_persistence() {
    ...
    // --- SESIÓN 1: APRENDIZAJE ---
    {
        ...
        assert!(
            kept_count >= 5,
            "Session 1 should show initial lines while learning. Got: {}",
            kept_count
        );
    ...
    // --- VERIFICACIÓN DE DB ---
    {
        ...
        assert!(
            templates
                .iter()
                .any(|(t, _)| t.contains("Status: Processing hash <HEX>...")),
    ...
    // --- SESIÓN 2: SABIDURÍA ---
    {
        ...
        assert!(
            result.is_none(),
            "Session 2 should instantly collapse known patterns from line 1"
        );
        ...
        let result_security = session.engine.process_line(
        ...
        assert!(
            result_security.is_some(),
            "Known patterns must NOT collapse if they contain secrets (V3 Priority)"
        );
