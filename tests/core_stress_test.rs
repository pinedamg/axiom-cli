...
fn test_core_generic_scenarios() {
    ...
    let traceback_line =
        "  File \"/usr/lib/python3.9/site-packages/requests/api.py\", line 64, in get";
    ...
    let result = session.engine.process_line(
        TerminalEvent::StaticLine(error_line.to_string()),
        command,
        &context_error,
    );
    ...
    let result_privacy = session.engine.process_line(
        TerminalEvent::StaticLine(secret_line.to_string()),
        command,
        &context_generic,
    );
...
fn test_aggregator_no_information_loss() {
    ...
    assert!(
        !summaries.is_empty(),
        "Aggregator should have captured noise lines"
    );
...
fn test_adversarial_secrets() {
    ...
    let result_git = session
        .engine
        .redactor
        .redact(&format!("commit {}", git_sha));
    assert!(result_git.contains(git_sha), "Git SHA falsely redacted");

    let result_docker = session
        .engine
        .redactor
        .redact(&format!("container {}", docker_id));
    assert!(
        result_docker.contains(docker_id),
        "Docker ID falsely redacted"
    );
}
