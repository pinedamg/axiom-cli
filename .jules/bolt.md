...
- **Regex Overhead Avoidance**: When regex replacements are used only to count matches or simplify strings (e.g., `extract_parts`), avoiding the extraction and storage of replaced variables into `Vec<String>` structures significantly reduces heap allocations, especially in line-by-line processing loops. Storing only match counts instead of variables into `BTreeMap<String, usize>` further decreases memory consumption.
- **Buffer Reuse over `mem::take`**: In hot-path stream filters (e.g., `StreamPipeline::process` and `stage_deduplicate`), calling `std::mem::take` on a `String` buffer correctly yields the string but leaves an empty string with 0 capacity behind, causing the next iteration to reallocate memory. Instead, extracting the data via `.clone()` followed immediately by `.clear()` keeps the underlying heap capacity allocated, resulting in far fewer reallocations at the cost of one temporary copy. (Similarly, `.take().unwrap_or_default()` and then `.clear()` for `Option<String>` achieves buffer reuse).
