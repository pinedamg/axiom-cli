...
*   **Memory Buffer Reuse (`src/engine/mod.rs`)**: Refactored `stage_deduplicate` to reuse existing `Option<String>` buffers using `.take()`, `.clear()`, and `.push_str()` instead of allocating a new `String` on every deduplication tick.
*   **Slice Reference Tracking (`src/engine/commands/ps.rs`)**: Refactored `generate_insight` to track the top process name using an `Option<&str>` referencing the `DiscoveryBuffer` instead of repeatedly cloning `String`s in a loop, minimizing heap thrashing. Additionally, replaced fragile `.unwrap()` chains in `parse_line` with `.unwrap_or(base)` to prevent panics and avoid creating intermediate structures.
