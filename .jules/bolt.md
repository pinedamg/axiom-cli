...
### 🧩 Structural Memory Audits & Optimizations (Hot Paths)
*   **Variable Buffer Overhaul (`src/engine/discovery.rs`)**: Refactored `variable_buffer` from tracking dense structures (`BTreeMap<String, Vec<Vec<String>>>`) to tracking match occurrences directly (`BTreeMap<String, usize>`). The actual memory overhead generated by storing large variable extraction arrays heavily impacted RAM limits under large log file flows. We no longer accumulate this unnecessary overhead. Memory measurement (`get_saved_bytes`) was adapted to rely on `capacity()` and value byte measurements accurately evaluating actual footprint bounds.
*   **Buffer String Re-allocations (`src/engine/mod.rs`)**: Resolved an issue inside `stage_deduplicate` where deduplication pipelines constantly allocated separate `String` instances on every iteration. Adapted to securely `take()` the previous iteration string, clear its content using `.clear()` and replace it with the new value by executing `.push_str()` - avoiding cyclic `String::new()` OS memory provisioning.
*   **WasmPluginManager Encapsulation (`src/engine/mod.rs`)**: Found `plugins` footprint natively increasing the engine structure size on initialization. Enclosed `WasmPluginManager` inside a `Box` dynamically storing the `plugins` field structure off the stack allocating its pointer cleanly reducing struct footprint impact.
*   **Gateway Buffer Sizing (`src/gateway/filters.rs`)**: `process` buffer cleared previously via `std::mem::take()` resetting string boundaries effectively to 0 capacity causing the loop logic to dynamically expand and copy characters repetitively per character on log streams. Refactored logic to utilize `std::mem::replace` with explicitly scaled pre-allocation `String::with_capacity(1024)`.

**Impact**: Massively reduced RAM usage generated by extracting/collapsing match tracking arrays alongside structural sizing improvements that eliminate micro-growth bottlenecks in deep string parsing pipelines.
